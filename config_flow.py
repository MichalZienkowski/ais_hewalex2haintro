...
class AisNbpConfigFlow(config_entries.ConfigFlow, domain="ais_hewalex2haintro"):
        ...
        if user_input is not None:
            ...
            options = {
                vol.Optional("currency"): cv.multi_select(
                    {"CHF": "Frank szwajcarski ", "EUR": "Euro",
                     "GBP": "Funt szterling", "USD": "Dolar amerykańsk"})
            }
            return self.async_show_form(step_id="settings", data_schema=vol.Schema(options))
    ...
    async def async_step_settings(self, user_input=None):
        ...
        if user_input is not None:
            ...
            return self.async_create_entry(title="Śledzenie ceny złota i kursów walut", data=user_input)
